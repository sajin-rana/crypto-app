...
const HomePortifolio = ({ isDark }: { isDark: boolean }) => {
  ...
  const darkHomeSelected = isHome ? "#ffff" : "#FFFFFF80";
  const lightHomeSelected = isHome ? "#353570" : "#9A9AB9";

  const darkPortfolioSelected = isPortfolio ? "#ffff" : "#FFFFFF80";
  const lightPortfolioSelected = isPortfolio ? "#353570" : "#9A9AB9";
      ...
      <div className="flex items-center gap-[24px] ">
        <Link href="/">
          <div className="flex items-center gap-[10px]">
            ...
            <p className="w-500 text-[16px] m-0 " style={{ color: homeColor }}>
              Home
            </p>
          </div>
        </Link>
